...
<template>
  <main class="content-area">
    <div v-if="!store.showPreview && !store.showStats && !store.showCharts && !store.showAdvanced && !store.showTTest && !store.showAiSummary && !store.showRadar" class="empty-state glass-card">
      <h2>📊 数据大屏就绪</h2><p>请在左侧点击按钮开启对应面板</p>
    </div>

    <div v-if="store.showAiSummary" class="glass-card result-panel ai-panel">
        <div class="panel-header"><h3 class="panel-title" style="color:#b37feb;">🤖 AI 智能数据解读报告</h3></div>
    ...
    <div v-if="store.showRadar && store.radarResult" class="glass-card result-panel">
        <div class="panel-header"><h3 class="panel-title" style="color:#e6a23c;">🕸️ 个体能力定位雷达图</h3></div>
    ...
    <div v-show="store.showCharts && store.showVisControl" class="drag-modal glass-card" :style="{ left: dragX + 'px', top: dragY + 'px' }">
        <div class="drag-header" @mousedown="startDrag"><span>⚙️ 控制图表显隐</span><button @click="store.showVisControl = false" class="close-btn">✕</button></div>
        <div class="drag-content"><p style="font-size:0.8rem; color:#888; margin-bottom:10px;">请勾选需要在右侧大屏显示的图表：</p><div class="checkbox-group"><label v-for="col in store.selectedVars" :key="col" class="checkbox-label" style="padding: 4px 10px;"><input type="checkbox" v-model="store.visActiveVars" :value="col" /> {{ col }}</label><label v-for="cat in store.catChartsData" :key="'cb-'+cat.variable" class="checkbox-label" style="padding: 4px 10px;"><input type="checkbox" v-model="store.visActiveVars" :value="cat.variable" /> {{ cat.variable }} (分类)</label></div></div>
    </div>
    ...
    <div v-if="store.showStats && store.statsResult" class="glass-card result-panel">
      <div class="panel-header"><h3 class="panel-title">🧮 描述性统计结果</h3><button @click="exportStats" class="glass-btn secondary-btn export-btn">⬇️ 导出 CSV</button></div>
      <div class="table-responsive"><table class="glass-table"><thead><tr><th>变量</th><th>均值</th><th>中位数</th><th>标准差</th><th>最小</th><th>最大</th></tr></thead><tbody><tr v-for="row in store.statsResult" :key="row.variable"><td class="var-name">{{ row.variable }}</td><td>{{ row.mean }}</td><td>{{ row.median }}</td><td>{{ row.std }}</td><td>{{ row.min }}</td><td>{{ row.max }}</td></tr></tbody></table></div>
    </div>

    <div v-if="store.showTTest && store.ttestResult" class="glass-card result-panel">
      <div class="panel-header"><h3 class="panel-title" style="color:#ff7875;">⚖️ 独立样本 t 检验 (分组: {{ store.selectedGroupVar }})</h3><button @click="exportTTest" class="glass-btn secondary-btn export-btn">⬇️ 导出 CSV</button></div>
      <div class="table-responsive"><table class="glass-table"><thead><tr><th>分析变量</th><th>组1均值</th><th>组2均值</th><th>t 值</th><th>P 值</th><th>结论</th></tr></thead><tbody><tr v-for="(res, index) in store.ttestResult" :key="index"><td class="var-name">{{ res.variable }}</td><td>{{ res.group1_mean }}</td><td>{{ res.group2_mean }}</td><td>{{ res.t_value }}</td><td>{{ res.p_value }}</td><td :class="res.significant ? 'danger-text' : 'success-text'">{{ res.significant ? '🔥 显著 (p<0.05)' : '➖ 不显著' }}</td></tr></tbody></table></div>
    </div>
    ...
    <div v-if="store.showAdvanced && store.advancedResult" class="glass-card result-panel">
      <div class="panel-header"><h3 class="panel-title" style="color:#b37feb;">🔬 统计检验与关联分析</h3><button @click="exportNormality" class="glass-btn secondary-btn export-btn">⬇️ 导出 CSV</button></div>
      <table class="glass-table mb-3"><thead><tr><th>变量</th><th>W 值</th><th>P 值</th><th>Shapiro-Wilk 结论</th></tr></thead><tbody><tr v-for="(res, index) in store.advancedResult.normality" :key="index"><td class="var-name">{{ res.variable }}</td><td>{{ res.statistic }}</td><td>{{ res.p_value }}</td><td :class="res.is_normal ? 'success-text' : 'danger-text'">{{ res.is_normal ? '✔️ 符合正态' : '❌ 不符合正态' }}</td></tr></tbody></table>
