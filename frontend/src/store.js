...
export const actions = {
  ...
  showDialog(options) {
    ...
    store.dialog.type = options.type || 'alert'; // 'alert' 或 'confirm'
  ...
  async uploadFile(file) {
    ...
    } catch (err) { actions.showDialog({ title: '❌ 上传失败', message: '文件上传或解析失败，请检查文件格式。' }); }
  },
  ...
  async triggerStandardization() {
    actions.showDialog({
      type: 'confirm', title: '⚙️ 确认标准化', message: '确定要执行 Z-score 标准化吗？\n\n此操作将消除量纲差异，将数值转化为均值为0的分布。',
      onConfirm: async () => { try { const res = await axios.post('http://127.0.0.1:5000/api/standardize', { filename: store.cleanResult.cleaned_filename }); if(res.data.status === 'success') { store.currentDataFile = res.data.data.std_filename; store.isStandardized = true; store.showPreview = false; } } catch (err) {} }
    });
  },
  undoStandardization() {
    actions.showDialog({
      type: 'confirm', title: '↩️ 撤回标准化', message: '确定要撤回标准化吗？\n\n数据将恢复为原始的常规数值。',
      onConfirm: () => { store.currentDataFile = store.cleanResult.cleaned_filename; store.isStandardized = false; store.showStats = false; store.showCharts = false; store.showAdvanced = false; store.showTTest = false; }
    });
  },

  async runDescriptiveStats() { if (store.showStats) { store.showStats = false; return; } if (store.selectedVars.length === 0) return actions.showDialog({ title: '提示', message: '请在左侧至少勾选一个数值变量！' }); try { const res = await axios.post('http://127.0.0.1:5000/api/analyze/descriptive', { filename: store.currentDataFile, columns: store.selectedVars }); if(res.data.status === 'success') { store.statsResult = res.data.data; store.showStats = true; } } catch (err) {} },
  async runTTest() { if (store.showTTest) { store.showTTest = false; return; } if (store.selectedVars.length === 0 || !store.selectedGroupVar) return actions.showDialog({ title: '提示', message: '缺少必要变量！请勾选数值变量并选择分组变量。' }); try { const res = await axios.post('http://127.0.0.1:5000/api/analyze/ttest', { filename: store.currentDataFile, group_col: store.selectedGroupVar, columns: store.selectedVars }); if(res.data.status === 'success') { store.ttestResult = res.data.data; store.showTTest = true; } else { actions.showDialog({ title: '计算失败', message: res.data.message }); } } catch (err) {} },
  ...
  renderCharts() {
    ...
    const initChart = (domId, options) => {
      ...
      if (dom) {
        ...
        chart.clear(); // 核心修复：每次渲染前先清空画布，强制触发优雅的入场动画！
        chart.setOption({...options, toolbox, animation: true, animationDuration: 1000, animationEasing: 'cubicOut'});
      }
    }
  ...
  async generateCharts() {
    ...
    if (store.selectedVars.length === 0) return actions.showDialog({ title: '提示', message: '请在左侧至少勾选一个数值变量！' });
    try {
  ...
  async runAdvancedAnalysis() {
    ...
    if (store.selectedVars.length < 2) return actions.showDialog({ title: '提示', message: '高级关联分析至少需要勾选 2 个变量！' });
    try {
  ...
  async triggerCleanup() {
    actions.showDialog({
      type: 'confirm', title: '🧹 清理系统缓存', message: '确定要清理系统缓存吗？\n\n此操作将删除所有已上传和分析的数据，且不可恢复。',
      onConfirm: async () => { try { const res = await axios.post('http://127.0.0.1:5000/api/cleanup'); if(res.data.status === 'success') { actions.showDialog({title:'清理成功', message: res.data.message}); store.isEntered = false; store.fileInfo = null; } } catch (err) {} }
    });
  }
}